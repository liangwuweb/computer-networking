...
def run_test_case(case: TestCase) -> TestResult:
    ...
    try:
        subprocess.run(
            test_args, timeout=half_credit_time, check=True, stdout=subprocess.PIPE
        )
...
for CASE_NUM, A_CASE in enumerate(TEST_CASES):
    ...
    if CASE_RESULT.type == TestResultType.TRANSFER_ERROR:
        ...
        print(
            "  ! 0 points, file not transferred in {} secs".format(
        ...
        print(
            "  * 2 points, transferred in {} secs".format(round(CASE_RESULT.duration))
        )
        TOTAL_SCORE += 2
    print("")
